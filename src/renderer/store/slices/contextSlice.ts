...
function validateSnapshot(
  ...
  // (Legacy: session tab type removed; this filter is a no-op for current Tab types)
  const validTabs = snapshot.openTabs.filter((tab) => {
    const legacyTab = tab as unknown as { type: string; projectId?: string };
    if (legacyTab.type === 'session' && legacyTab.projectId) {
      return validProjectIds.has(legacyTab.projectId) || validWorktreeIds.has(legacyTab.projectId);
    }
    return true;
  });
  ...
  const validatedPanes = snapshot.paneLayout.panes
    .map((pane) => {
      const paneTabs = pane.tabs.filter((tab) => {
        ...
        if (legacyTab.type === 'session' && legacyTab.projectId) {
          return (
            validProjectIds.has(legacyTab.projectId) || validWorktreeIds.has(legacyTab.projectId)
          );
        }
        return true;
      });
