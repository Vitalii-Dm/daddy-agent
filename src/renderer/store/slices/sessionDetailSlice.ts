...
// Legacy session tab fields — session tab type has been removed but these code paths
// remain for backward compatibility. Cast tabs to this type when checking legacy fields.
interface LegacySessionTab {
  id: string;
  type: string;
  sessionId?: string;
  projectId?: string;
...
export const createSessionDetailSlice: StateCreator<AppState, [], [], SessionDetailSlice> = (
  ...
  fetchSessionDetail: async (
    ...
    try {
      ...
      // Session tabs no longer exist; tab label update is a no-op.
      ...
      const activeTab = currentState.getActiveTab() as LegacySessionTab | null;
  ...
  refreshSessionInPlace: async (projectId: string, sessionId: string) => {
    ...
    const allTabs = getAllTabs(currentState.paneLayout) as LegacySessionTab[];
    ...
    try {
      ...
      const latestAllTabs = getAllTabs(latestState.paneLayout) as LegacySessionTab[];
      const stillViewingSession =
