...
async function refreshTaskChangePresenceForUpdatedTask(
  ...
  // invalidateTaskChangePresence / checkTaskHasChanges were removed with changeReviewSlice
  const stateAny = state as unknown as Record<string, unknown>;
  if (
    typeof stateAny.invalidateTaskChangePresence !== 'function' ||
    typeof stateAny.checkTaskHasChanges !== 'function'
  ) {
    return;
  }

  const cacheKey = buildTaskChangePresenceKey(teamName, taskId, options);
  (stateAny.invalidateTaskChangePresence as (keys: string[]) => void)([cacheKey]);

  try {
    await (stateAny.checkTaskHasChanges as (a: string, b: string, c: unknown) => Promise<void>)(
      teamName,
      taskId,
      options
    );
  ...
  selectTeam: async (teamName: string, opts) => {
    ...
    try {
      ...
      if (invalidationState.cacheKeys.length > 0) {
        // invalidateTaskChangePresence removed with changeReviewSlice
      }
