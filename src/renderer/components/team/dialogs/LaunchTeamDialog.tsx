...
export const LaunchTeamDialog = (props: LaunchTeamDialogProps): React.JSX.Element => {
  ...
  const [prepareState, setPrepareState] = useState<'idle' | 'loading' | 'ready' | 'failed'>('ready');
  ...
  // Schedule store actions (schedule slice removed — these are stubs)
  const createSchedule = async (_input: unknown): Promise<void> => {};
  const updateSchedule = async (_id: string, _patch: unknown): Promise<void> => {};
